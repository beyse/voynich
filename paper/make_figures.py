"""Figures for the paper, computed from results/mechanism/*.json (run from repo root)."""
import json
import os
...
# ---------------------------------------------------------------- profile figure (Section 3), from paper/numbers.json
def fig_profile():
    N = json.load(open('paper/numbers.json'))
    pj, lx = N['proj'], N['lexicon']
    fig, (a, b) = plt.subplots(1, 2, figsize=(7.0, 2.9), gridspec_kw={'width_ratios': [1.15, 1]})
    rows = [('VMS c=jn', 'Voynich'), ('VMSA c=jn', 'Currier A'), ('VMSB c=jn', 'Currier B'),
            ('Latin', 'Latin'), ('Italian', 'Italian'), ('German', 'German'), ('Hebrew', 'Hebrew')]
    x = np.arange(len(rows))
    w = 0.38
    a.bar(x - w / 2 - 0.01, [pj[k]['l1f1'] for k, _ in rows], w, color=S1, label='boundary glyphs')
    a.bar(x + w / 2 + 0.01, [pj[k]['word'] for k, _ in rows], w, color=S2, label='whole tokens')
    a.set_xticks(x)
    a.set_xticklabels([n for _, n in rows], rotation=30, ha='right')
    a.set_ylabel('excess MI between adjacent tokens (bit)')
    a.grid(axis='x', visible=False)
    a.axvline(2.5, color=INK2, lw=0.6, ls=':')
    a.set_ylim(0, 0.56)
    a.legend(loc='upper left', fontsize=7.5)
    a.set_title('a  Where the ordering information sits', loc='left', fontsize=9)
    rows = [('VMS', 'Voynich'), ('VMS A', 'Currier A'), ('VMS B', 'Currier B'), ('Latin', 'Latin'), ('Italian', 'Italian'),
            ('German', 'German'), ('Hebrew', 'Hebrew'), ('Latin rule msty', 'Latin, rule-spaced'), ('Latin bisyl', 'Latin, syllable pairs')]
    y = np.arange(len(rows))[::-1]
    real = [lx[k]['2']['tok_real'] for k, _ in rows]
    syn = [lx[k]['2']['tok_synth'] for k, _ in rows]
    for yy, r, s_ in zip(y, real, syn):
        b.plot([s_, r], [yy, yy], color=INK2, lw=1.0, zorder=2)
    b.scatter(real, y, color=S1, s=30, zorder=3, edgecolor='white', linewidth=1, label='real, held out')
    b.scatter(syn, y, color=S2, s=30, zorder=3, marker='s', edgecolor='white', linewidth=1, label='sampled from model')
    b.set_yticks(y)
    b.set_yticklabels([n for _, n in rows])
    b.set_xlim(0, 1)
    b.set_xlabel('share attested in the training half')
    b.grid(axis='y', visible=False)
    b.axhline(y[2] - 0.5, color=INK2, lw=0.6, ls=':')
    b.legend(loc='upper left', fontsize=7.5, borderaxespad=0.2)
    b.set_title('b  Lexicon test', loc='left', fontsize=9)
    fig.tight_layout(w_pad=2.0)
    save(fig, 'fig_profile')


if __name__ == '__main__':
    for f in (fig_profile, fig1, fig2, fig3, fig4, fig5, fig6):
        f()
        print('ok', f.__name__)
